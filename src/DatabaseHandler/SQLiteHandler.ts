...
export class SQLiteDbHandler {
    ...
    */
    ...
    public async getCases(query: string): Promise<Case[]> {
        ...
        var cases: Case[] = await new Promise((resolve, reject) => {
            ...
            this.db.all(query, function (err, rows) {
                ...
                var cases: Case[] = [];
                rows.forEach(row => {
                    var hearing = new Hearing(row["rowid"],row["nextHearingDate"] , row["proceedings"]);
                    var myCase: Case = new Case(row["rowid"], row["title"], row["judge"], row["type"], row["onBehalfOf"], row["status"], row["description"], "NA", hearing);
                    cases.push(myCase);
